...
def gaussian_noise(img):
    stddev = np.random.uniform(0.5,1)
...
class NoiseDataLoader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        x = np.zeros((self.patch_num, self.image_size , self.image_size , 3), dtype=np.uint8)
        y = np.zeros((self.patch_num, self.image_size , self.image_size , 3), dtype=np.uint8)
        ...
        while True:
            ...
            if h >= image_h and w >= image_w:
                ...
                if sample_id == self.patch_num:
                    return AB_path , x.reshape(self.patch_num,3,self.image_size , self.image_size) , y.reshape(self.patch_num,3,self.image_size , self.image_size)  # X : Noisy Input / Y : Clean Target
    ...
    def __getitem__(self, index):
        ...
        while True:
            ...
            if h >= image_h and w >= image_w:
                ...
                if sample_id == self.patch_num:
                    return AB_path , x.reshape(self.patch_num,1,self.image_size , self.image_size) , y.reshape(self.patch_num,1,self.image_size , self.image_size)  # X : Noisy Input / Y : Clean Target
